...
class InMemoryDMNExecutorTest {
    ...
    void testExecute() throws Exception {
        ...
        // Output path
        Path outputPath = Paths.get("target", "in-memory", "dmn");
        File outputSourceFolder = outputPath.resolve("dmn").toFile();
        File outputTestFolder = outputPath.resolve("tck").toFile();

        runTests(inputModelFile, inputTestFile, outputSourceFolder, outputTestFolder);
    }

    private void runTests(File inputModelFile, File inputTestFile, File outputSourceFolder, File outputTestFolder) throws Exception {
