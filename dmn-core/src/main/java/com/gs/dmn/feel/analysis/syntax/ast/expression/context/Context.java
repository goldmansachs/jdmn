...
public class Context<T> extends Expression<T> {
    private final List<ContextEntry<T>> entries = new ArrayList<>();

    public Context(List<ContextEntry<T>> entries) {
        if (entries != null) {
            this.entries.addAll(entries);
        }
    }

    public List<ContextEntry<T>> getEntries() {
        return this.entries;
    }

    public ContextEntry<T> entry(String name) {
        List<ContextEntry<T>> result = this.entries.stream().filter(e -> name.equals(e.getKey().getKey())).collect(Collectors.toList());
        return result.size() == 1 ? result.get(0) : null;
    }

    public Map<String, Object> toMap() {
        Map<String, Object> result = new LinkedHashMap<>();
        for(ContextEntry<T> entry: this.entries) {
            String key = entry.getKey().getKey();
            Expression<T> expression = entry.getExpression();
            Object value;
            if (expression instanceof Context) {
                value = ((Context<T>) expression).toMap();
