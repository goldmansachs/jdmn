...
class StandardFEELInterpreterVisitor<NUMBER, DATE, TIME, DATE_TIME, DURATION> extends AbstractFEELInterpreterVisitor<NUMBER, DATE, TIME, DATE_TIME, DURATION> {
    ...
    @Override
    protected Object evaluateDateTimeMember(Object source, String member, ModelCoordinates coordinates) {
        ...
        if ("year".equals(member)) {
            ...
            SemanticError error = ErrorFactory.makeExpressionError(coordinates, String.format("Cannot resolve method '%s' for date time", member));
            handleError(error);
            return null;
        }
    }
