...
public class DMNToNativeVisitor extends TraversalVisitor<NativeVisitorContext> {
    ...
    public DMNToNativeVisitor(BuildLogger logger, LogErrorHandler errorHandler, BasicDMNToNativeTransformer<Type, DMNContext> dmnTransformer, DMNModelRepository dmnModelRepository, TemplateProcessor templateProcessor, OutputRepository outputRepository, List<String> generatedClasses, String decisionBaseClass) {
    ...
    public DMNBaseElement visit(TItemDefinition element, NativeVisitorContext context) {
        ...
        try {
            ...
            if (dmnTransformer.getDMNModelRepository().hasComponents(element)) {
                ...
                transformItemDefinition(definitions, element, dmnTransformer, templateProcessor.getTemplateProvider().baseTemplatePath(), templateProcessor.getTemplateProvider().itemDefinitionInterfaceTemplate(), generatedClasses, outputRepository, typePackageName, nativeInterfaceName);
                transformItemDefinition(definitions, element, dmnTransformer, templateProcessor.getTemplateProvider().baseTemplatePath(), templateProcessor.getTemplateProvider().itemDefinitionClassTemplate(), generatedClasses, outputRepository, typePackageName, dmnTransformer.itemDefinitionNativeClassName(nativeInterfaceName));
    ...
    private void transformItemDefinition(TDefinitions definitions, TItemDefinition itemDefinition, BasicDMNToNativeTransformer<Type, DMNContext> dmnTransformer, String baseTemplatePath, String itemDefinitionTemplate, List<String> generatedClasses, OutputRepository outputRepository, String typePackageName, String typeName) {
        ...
        if (generatedClasses.contains(qualifiedName)) {
            ...
            templateProcessor.processTemplate(definitions, itemDefinition, baseTemplatePath, itemDefinitionTemplate, dmnTransformer, outputRepository, typePackageName, typeName);
    ...
    public DMNBaseElement visit(TBusinessKnowledgeModel element, NativeVisitorContext context) {
        ...
        try {
            ...
            templateProcessor.processTemplate(definitions, element, templateProcessor.getTemplateProvider().baseTemplatePath(), templateProcessor.getTemplateProvider().bkmTemplateName(), dmnTransformer, outputRepository, bkmPackageName, bkmClassName, decisionBaseClass);
            ...
            if (dmnTransformer.getDMNModelRepository().isDecisionTableExpression(element)) {
                ...
                templateProcessor.processTemplate(definitions, element, templateProcessor.getTemplateProvider().baseTemplatePath(), templateProcessor.getTemplateProvider().decisionTableRuleOutputTemplate(), dmnTransformer, outputRepository, bkmPackageName, decisionRuleOutputClassName, decisionBaseClass);
            }
    ...
    public DMNBaseElement visit(TDecisionService element, NativeVisitorContext context) {
        ...
        try {
            ...
            templateProcessor.processTemplate(definitions, element, templateProcessor.getTemplateProvider().baseTemplatePath(), templateProcessor.getTemplateProvider().dsTemplateName(), dmnTransformer, outputRepository, dsPackageName, dsClassName, decisionBaseClass);
    ...
    public DMNBaseElement visit(TDecision element, NativeVisitorContext context) {
        ...
        try {
            ...
            templateProcessor.processTemplate(definitions, element, templateProcessor.getTemplateProvider().baseTemplatePath(), templateProcessor.getTemplateProvider().decisionTemplateName(), dmnTransformer, outputRepository, decisionPackageName, decisionClassName, decisionBaseClass);

            if (dmnTransformer.getDMNModelRepository().isDecisionTableExpression(element)) {
                String decisionRuleOutputClassName = dmnTransformer.ruleOutputClassName(element);
                checkDuplicate(generatedClasses, decisionPackageName, decisionRuleOutputClassName, dmnTransformer);
                templateProcessor.processTemplate(definitions, element, templateProcessor.getTemplateProvider().baseTemplatePath(), templateProcessor.getTemplateProvider().decisionTableRuleOutputTemplate(), dmnTransformer, outputRepository, decisionPackageName, decisionRuleOutputClassName, decisionBaseClass);
            }
    ...
    private void visitForInputs(TDRGElement element, NativeVisitorContext context) {
        ...
        try {
            ...
            if (pojoTemplateName != null) {
                templateProcessor.processTemplate(definitions, element, templateProcessor.getTemplateProvider().baseTemplatePath(), pojoTemplateName, dmnTransformer, outputRepository, inputPackageName, inputClassName, decisionBaseClass);
            }
        } catch (Exception e) {
            SemanticError error = makeError(definitions, element);
            throw new SemanticErrorException(error.toText(), e);
