...
@Mojo(name = "tck-to-python", defaultPhase = LifecyclePhase.GENERATE_TEST_SOURCES, configurator = "dmn-mojo-configurator")
public class TCKToPythonJUnitMojo<NUMBER, DATE, TIME, DATE_TIME, DURATION> extends AbstractTestToJunitMojo<NUMBER, DATE, TIME, DATE_TIME, DURATION, TestCases> {
    @Parameter(required = true, defaultValue = "com.gs.dmn.dialect.PythonStandardDMNDialectDefinition")
    public String dmnDialect;

    @Parameter(required = true, defaultValue = "com.gs.dmn.transformation.template.PythonTreeTemplateProvider")
    ...
    protected FileTransformer makeTransformer(DMNDialectDefinition<NUMBER, DATE, TIME, DATE_TIME, DURATION, TestCases> dmnDialect, DMNValidator dmnValidator, DMNTransformer<TestCases> dmnTransformer, TemplateProvider templateProvider, LazyEvaluationDetector lazyEvaluationDetector, TypeDeserializationConfigurer typeDeserializationConfigurer, InputParameters inputParameters, BuildLogger logger) {
        return new TCKTestCasesToPythonJUnitTransformer<>(
                dmnDialect,
                dmnValidator,
                dmnTransformer,
                templateProvider,
