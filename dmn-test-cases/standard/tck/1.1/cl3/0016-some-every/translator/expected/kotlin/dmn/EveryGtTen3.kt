...
class EveryGtTen3(val priceTable1 : PriceTable1 = PriceTable1()) : com.gs.dmn.runtime.JavaTimeDMNBaseDecision() {
    ...
    private inline fun evaluate(context_: com.gs.dmn.runtime.ExecutionContext): Boolean? {
        ...
        return booleanAnd(priceTable1?.stream()?.map({ i -> booleanEqual(GtTen.instance()?.apply(i?.let({ it.price as java.lang.Number? }), context_), true) })?.collect(Collectors.toList())?.toList()) as Boolean?
    }
